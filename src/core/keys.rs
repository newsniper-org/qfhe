use crate::core::{polynomial::Polynomial, Ciphertext, GgswCiphertext, SecurityLevel, SecretKey, PublicKey, RelinearizationKey, BootstrapKey, KeySwitchingKey};
...
pub fn generate_keys<B : HardwareBackend>(level: SecurityLevel, master_key: &MasterKey, salt: &Salt, backend: &B) -> (SecretKey, PublicKey, KeySwitchingKey, RelinearizationKey, BootstrapKey) {
    ...
    let rlk = backend.generate_relinearization_key(&sk, &mut rng, &params);
    let ksk = backend.generate_keyswitching_key(&sk, &sk, &mut rng, &params);
    let bk = backend.generate_bootstrap_key(&sk, &mut rng, &params);
